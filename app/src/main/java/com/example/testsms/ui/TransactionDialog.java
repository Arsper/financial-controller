...
public class TransactionDialog {
    ...
    public static void show(Context ctx,
        ...
        // Adapters for operation types
        ArrayAdapter<CharSequence> typeAdapterDefault = ArrayAdapter.createFromResource(
                ctx, R.array.operation_types, android.R.layout.simple_spinner_item
        );
        typeAdapterDefault.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);

        ArrayAdapter<CharSequence> typeAdapterCard = ArrayAdapter.createFromResource(
                ctx, R.array.operation_types_card, android.R.layout.simple_spinner_item
        );
        typeAdapterCard.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ...
        spinnerBalanceType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selectedBalanceType = parent.getItemAtPosition(position).toString();
                if (selectedBalanceType.equals("Карта")) {
                    spinnerType.setAdapter(typeAdapterCard);
                } else {
                    spinnerType.setAdapter(typeAdapterDefault);
                }
            }
        ...
        if (!isNew) {
            ...
            int catPos = (isIncome ? catAdapterIncome : catAdapterDefault).getPosition(info.category);
            if (catPos >= 0) spinnerCategory.setSelection(catPos);

            int typePos;
            if (info.balanceType.equals("Карта")) {
                typePos = typeAdapterCard.getPosition(info.type);
            } else {
                typePos = typeAdapterDefault.getPosition(info.type);
            }
        ...
        new AlertDialog.Builder(ctx)
                ...
                .setPositiveButton(isNew ? "Добавить" : "Сохранить", (dlg, which) -> {
                    ...
                    info.description = etDescription.getText().toString().trim();

                    Log.d("DEBUG_DESC", "Saved transaction description: " + info.description);
